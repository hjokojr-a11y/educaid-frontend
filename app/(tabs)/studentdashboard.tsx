...
// Pure RN African pattern background
function AfricanBg() {
  const spots = [
    { top: 60,  left: 16,       size: 44, color: C.navy  },
    { top: 200, left: width-64, size: 36, color: C.green },
    { top: 360, left: 24,       size: 40, color: C.green },
    { top: 520, left: width-58, size: 46, color: C.navy  },
    { top: 680, left: 12,       size: 38, color: C.navy  },
  ];
  return (
      ...
      {spots.map((s, i) => {
        ...
        return (
          <View key={i} style={{ position: 'absolute', top: s.top, left: s.left, width: s.size, height: s.size }}>
            <View style={{ position: 'absolute', top: 0, left: 0, width: s.size, height: s.size, borderRadius: r, borderWidth: 1.8, borderColor: s.color, opacity: 0.065 }} />
            <View style={{ position: 'absolute', top: s.size*0.25, left: s.size*0.25, width: s.size*0.5, height: s.size*0.5, borderRadius: r*0.5, borderWidth: 1.2, borderColor: s.color, opacity: 0.065 }} />
            <View style={{ position: 'absolute', top: r-1, left: 0, width: s.size, height: 1.5, backgroundColor: s.color, opacity: 0.055 }} />
            <View style={{ position: 'absolute', top: 0, left: r-1, width: 1.5, height: s.size, backgroundColor: s.color, opacity: 0.055 }} />
          </View>
        );
      })}
    </View>
...
function BackBtn({ onPress }: any) {
  return (
    <TouchableOpacity onPress={onPress} style={SD.backBtn} activeOpacity={0.7}>
      <Text style={SD.backBtnTxt}>← Back</Text>
    </TouchableOpacity>
  );
}
...
export default function StudentDashboardScreen() {
  ...
  useEffect(() => {
    if (session) {
      const iv = setInterval(() => refresh(session.token, session.user), 30000);
      return () => clearInterval(iv);
    }
  }, [session]);

  async function refresh(token: string, user: any) {
  ...
  if (step === 'selectSchool') {
    return (
      <View style={[SD.fill, { backgroundColor: C.canvas }]}>
        <AfricanBg />
        <ScrollView contentContainerStyle={SD.pad}>
          <BackBtn onPress={() => setStep('login')} />
  ...
  if (step === 'login' && !session) {
    return (
      <View style={[SD.fill, { backgroundColor: C.canvas }]}>
        <AfricanBg />
        <ScrollView contentContainerStyle={SD.pad} keyboardShouldPersistTaps="handled">
          ...
          {selectedSchool ? (
            <TouchableOpacity style={[SD.schoolPill, { borderLeftColor: schoolAccent }]}
      ...
      {dataLoading
        ...
        : (
          <ScrollView style={SD.body}>
            ...
            {/* Section header line */}
            <View style={SD.sectionHd}>
              <Text style={[SD.sectionHdTxt, { color: activeTab?.color || C.navy }]}>
                {activeTab?.emoji}  {activeTab?.label?.toUpperCase()}
              </Text>
              <View style={[SD.sectionHdLine, { backgroundColor: activeTab?.color || C.navy }]} />
            </View>
            ...
            {tab === 'home' && (
              <View style={SD.statsGrid}>
                {[
                  { label:'Attendance', count:attendance.length,    color:C.green,  t:'attendance' },
                  { label:'Grades',     count:academic.length,      color:C.navy,   t:'academic'   },
                  { label:'Homework',   count:homework.length,      color:C.purple, t:'homework'   },
                  { label:'Sports',     count:sports.length,        color:C.amber,  t:'sports'     },
                  { label:'News',       count:announcements.length, color:C.teal,   t:'announce'   },
                  { label:'Alerts',     count:alerts.length,        color:C.red,    t:'alerts'     },
                ].map((s,i) => (
...
const SD = StyleSheet.create({
  ...
  sectionHd:     { flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 16, marginTop: 4 },
  sectionHdTxt:  { fontSize: 11, fontWeight: '700', letterSpacing: 1.2 },
  sectionHdLine: { flex: 1, height: 1.5, borderRadius: 1 },
